...
//	  404: errorGeneric
//	  default: errorGeneric
func (h *Handler) listIdentityDevices(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	iID, err := uuid.FromString(ps.ByName("id"))
	if err != nil {
		h.r.Writer().WriteError(w, r, errors.WithStack(herodot.ErrBadRequest.WithError(err.Error()).WithDebug("could not parse UUID")))
		return
...
//	  409: errorGeneric
//	  default: errorGeneric
func (h *Handler) patchIdentityDevices(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	id := x.ParseUUID(ps.ByName("id"))
	deviceId := x.ParseUUID(ps.ByName("device_id"))
