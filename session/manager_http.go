...
func (s *ManagerHTTP) ActivateSession(r *http.Request, session *Session, i *identity.Identity, authenticatedAt time.Time) (err error) {
	...
	s.r.Logger().WithField("current_device", currentDevice).WithField("devices", session.Devices).Info("activatesession: current_device")
...
func (s *ManagerHTTP) adjustSessionAAL(ctx context.Context, sess *Session, currentDevice *Device, i *identity.Identity) {
	...
	if currentDevice.Fingerprint != nil && (iaal > identity.AuthenticatorAssuranceLevel1) && (currentDevice.AMR == nil || len(currentDevice.AMR) == 0) { // Don't perform the list devices call if there's no fingerprint anyway
		...
		if trustedDevices, err = s.r.SessionPersister().ListTrustedDevicesByIdentity(ctx, i.ID); err == nil && len(trustedDevices) > 0 {
			s.r.Logger().WithField("session_trusted_devices", sess.TrustedDevices).WithField("trusted_devices", trustedDevices).WithField("current_device", currentDevice).Info("comparing current_device to trusted_devices")
					...
					for _, amr := range td.AMR {
						if _, ok := i.GetCredentials(amr.Method); ok {
							...
							if amr.AAL > sess.AuthenticatorAssuranceLevel && now.Before(trustDeviceExpiration) {
								s.r.Logger().WithField("amr", amr).WithField("trusted_device", td).Trace("device trusted; adding aal2+ for this trusted device")
								amr.DeviceTrustBased = true
								sess.CompletedLoginForMethod(amr)
								break
							} else if now.After(trustDeviceExpiration) {
								s.r.Logger().WithField("amr", amr).WithField("trusted_device", td).Trace("device trust is too old; not adding aal2+ for this trusted device")
							}
						}
					}
				}
