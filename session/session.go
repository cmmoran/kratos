...
func CurrentDeviceForRequest(r *http.Request) *Device {
	...
	if dev, ok := ctx.Value(DeviceKey).(*Device); ok {
		...
		dev = &Device{
			IPAddress: compatx.Ptr(httpx.ClientIP(r)),
			AMR:       make(AuthenticationMethods, 0),
		}

		agent := r.Header["User-Agent"]
		if len(agent) > 0 {
			dev.UserAgent = compatx.Ptr(strings.Join(agent, " "))
		}
		...
		dev.Location = compatx.Ptr(strings.Join(clientGeoLocation, ", "))

		if r.Header.Get("X-Session-Entropy") != "" {
			dev.Fingerprint = compatx.Ptr(r.Header.Get("X-Session-Entropy"))
		}
