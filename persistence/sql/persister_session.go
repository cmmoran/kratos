...
func (p *Persister) UpsertSession(ctx context.Context, s *session.Session) (err error) {
	...
	return errors.WithStack(p.Transaction(ctx, func(ctx context.Context, tx *pop.Connection) (err error) {
		...
		if exists {
			...
			for i := range s.Devices {
				...
				if device.TrustPending != nil {
					...
					p.r.Logger().WithField("device", device).Debug("updating trusted device")
		...
		for i := range s.Devices {
			...
			device.NID = s.NID
			device.IdentityID = compatx.Ptr(s.IdentityID)
			...
			if device.Fingerprint != nil {
				device.Fingerprint = compatx.Ptr(stringsx.TruncateByteLen(*device.Fingerprint, SessionDeviceFingerprintMaxLength))
			}
			if device.TrustPending != nil {
				device.Trusted = *device.TrustPending
				device.TrustPending = nil
			...
			p.r.Logger().WithField("device", device).Debug("creating device")
