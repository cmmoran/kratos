...
func (e *HookExecutor) PostLoginHook(
	...
	if f.Type == flow.TypeBrowser && x.IsJSONRequest(r) {
		e.d.Logger().WithRequest(r).WithField("identity_id", i.ID).WithField("continue_with", f.ContinueWith()).Info("Adding post login flow continue_with")
		f.AddContinueWith(flow.NewContinueWithRedirectBrowserTo(returnTo.String()))
		e.d.Logger().WithRequest(r).WithField("identity_id", i.ID).WithField("continue_with", f.ContinueWith()).Info("Add post login flow continue_with")
	}
	...
	if x.IsBrowserRequest(r) && f.HasContinueWithRedirect() {
		e.d.Logger().WithField("continue_with", f.ContinueWith()).WithField("identity_id", i.ID).WithField("flow_method", f.Active).Info("Redirecting to continue_with.")
