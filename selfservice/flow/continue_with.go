...
// FirstContinueWithRedirect returns the first non-empty redirect-capable
// continue_with action, using explicit priority instead of insertion order.
func FirstContinueWithRedirect(items []ContinueWith) (ContinueWithRedirect, bool) {
	for _, action := range []string{
		string(ContinueWithActionRedirectBrowserToString),
		string(ContinueWithActionShowVerificationUIString),
		string(ContinueWithActionShowSettingsUIString),
		string(ContinueWithActionShowRecoveryUIString),
	} {
		for _, item := range items {
			cwr, ok := item.(ContinueWithRedirect)
			if !ok || cwr.GetAction() != action || cwr.RedirectUrl() == "" {
				continue
			}
			return cwr, true
		}
