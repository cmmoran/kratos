...
func (s *Strategy) continueSettingsFlowUntrustDevice(ctx context.Context, ctxUpdate *settings.UpdateContext, p updateSettingsFlowWithTotpMethod) (*identity.Identity, error) {
	s.d.Logger().WithField("totp_device_untrust", p.DeviceUntrust).WithField("identity", ctxUpdate.Session.Identity.ID.String()).Info("untrusting totp mfa device for account")
	...
	s.d.Logger().Info("finished continueSettingsFlowTotpUntrustDevice")
