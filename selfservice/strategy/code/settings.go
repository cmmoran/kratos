...
func (s *Strategy) RegisterSettingsRoutes(_ *httprouterx.RouterPublic) {
}
...
func (s *Strategy) PopulateSettingsMethod(ctx context.Context, r *http.Request, i *identity.Identity, f *settings.Flow) (err error) {
	...
	if hasCode {
		s.deps.Logger().WithField("has_code", hasCode).Info("code credentials enabled for account")
		...
		s.deps.Logger().WithField("has_code", hasCode).Info("code credentials missing")
...
func (s *Strategy) continueSettingsFlow(ctx context.Context, r *http.Request, ctxUpdate *settings.UpdateContext, p updateSettingsFlowWithCodeMethod) error {
	...
	if len(p.DeviceUntrust) > 0 {
		...
		s.deps.Logger().WithField("code_device_untrust", p.DeviceUntrust).WithField("identity", ctxUpdate.Session.Identity.ID.String()).Info("untrusting device code code mfa")
	} else if p.CodeEnable {
		i, err = s.continueSettingsFlowEnable(ctx, ctxUpdate, p)
		s.deps.Logger().WithField("code_enable", p.CodeEnable).WithField("identity", ctxUpdate.Session.Identity.ID.String()).Info("code credentials should be enabled")
	} else if p.CodeDisable {
		i, err = s.continueSettingsFlowDisable(ctx, ctxUpdate, p)
		s.deps.Logger().WithField("code_disable", p.CodeDisable).WithField("identity", ctxUpdate.Session.Identity.ID.String()).Info("code credentials should be deleted")
	}
...
func (s *Strategy) continueSettingsFlowDeviceUntrust(ctx context.Context, ctxUpdate *settings.UpdateContext, p updateSettingsFlowWithCodeMethod) (*identity.Identity, error) {
	s.deps.Logger().WithField("code_device_untrust", p.DeviceUntrust).WithField("identity", ctxUpdate.Session.Identity.ID.String()).Info("untrusting code mfa device for account")
	...
	s.deps.Logger().Info("finished continueSettingsFlowCodeUntrustDevice")
...
func (s *Strategy) continueSettingsFlowEnable(ctx context.Context, ctxUpdate *settings.UpdateContext, p updateSettingsFlowWithCodeMethod) (*identity.Identity, error) {
	s.deps.Logger().WithField("code_enable", p.CodeEnable).WithField("identity", ctxUpdate.Session.Identity.ID.String()).Info("enabling code credentials for identity")
	...
	s.deps.Logger().Info("finished continueSettingsFlowEnable")
...
func (s *Strategy) continueSettingsFlowDisable(ctx context.Context, ctxUpdate *settings.UpdateContext, p updateSettingsFlowWithCodeMethod) (*identity.Identity, error) {
	...
	s.deps.Logger().Info("finished continueSettingsFlowDisable")
