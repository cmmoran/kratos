...
import (
	...
	"github.com/ory/kratos/x/compatx"
		...
		if p.TrustDevice {
			...
			if currentDevice.DeviceTrustConfidence(sess.Devices) > 0.0 {
				...
				if currentDevice.SessionID == uuid.Nil || currentDevice.SessionID != sess.ID {
					s.deps.Logger().WithRequest(r).WithField("current_device", *currentDevice).Warn("current device session id mismatch")
					(*currentDevice).SessionID = sess.ID
				}
				(*currentDevice).TrustPending = compatx.Ptr(p.TrustDevice)
				(*currentDevice).AMR = append((*currentDevice).AMR, method)
				s.deps.Logger().WithRequest(r).WithField("current_device", *currentDevice).WithField("devices", sess.Devices).WithField("amr", method).Debug("setting device to trusted")
