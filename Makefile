...
.PHONY: docker
docker:
	DOCKER_BUILDKIT=1 DOCKER_CONTENT_TRUST=1 docker build -f .docker/Dockerfile-build --build-arg=COMMIT=$(VCS_REF) --build-arg=BUILD_DATE=$(BUILD_DATE) -t ${DOCKER_FULL}:${IMAGE_TAG} .
...
.PHONY: post-release
post-release: .bin/yq
	cat quickstart.yml | yq '.services.kratos.image = "'$$DOCKER_FULL':'$$IMAGE_TAG'"' | sponge quickstart.yml
	cat quickstart.yml | yq '.services.kratos-migrate.image = "'$$DOCKER_FULL':'$$IMAGE_TAG'"' | sponge quickstart.yml
	cat quickstart.yml | yq '.services.kratos-selfservice-ui-node.image = "'$$DOCKER_REPO'/kratos-selfservice-ui-node:'$$IMAGE_TAG'"' | sponge quickstart.yml
